...
public sealed class GetContentItemBySlugQueryHandler : IRequestHandler<GetContentItemBySlugQuery, ContentItemDetailResponse>
{
    private readonly IContentItemRepository _contentItems;

    public GetContentItemBySlugQueryHandler(IContentItemRepository contentItems)
    {
        _contentItems = contentItems;
    }

    public async Task<ContentItemDetailResponse> Handle(GetContentItemBySlugQuery request, CancellationToken cancellationToken)
    {
        var contentItem = await _contentItems.GetBySlugAsync(request.Slug, cancellationToken);
        if (contentItem is null)
        {
            throw new ContentItemNotFoundException($"Content item with slug '{request.Slug}' was not found.");
        }
